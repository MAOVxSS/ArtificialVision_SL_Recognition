...
import pandas as pd
from Constants.constants import STATIC_MODEL_v3_LABELS_DIR, STATIC_MODEL_v3_KEYPOINTS_DIR, id_cam
...
def capture_keypoints(image, model):
    ...
    if results.multi_hand_landmarks:  # Si se detectan manos
        ...
        keypoints = np.array([[landmark.x, landmark.y] for landmark in hand_landmarks.landmark])  # Extraer los puntos clave
...
def save_data(keypoints, label, keypoints_file):
    ...
    try:
        with open(keypoints_file, 'a') as f:
            np.savetxt(f, [data], delimiter=',', fmt='%d' + ',%1.5f' * keypoints.size)  # Guardar los datos en el archivo
        print(f"Data saved for label: {label}")
    except PermissionError as e:  # Manejar el error de permiso
        print(f"Permission error: {e}")
        alternative_file = keypoints_file.replace(".csv", "_backup.csv")  # Guardar en un archivo de respaldo si hay error
...
def main():
    ...
    cap = cv2.VideoCapture(id_cam)  # Iniciar la captura de video desde la cámara
    ...
    while cap.isOpened():
        ...
        if hand_landmarks:  # Si se detectaron manos
            for hand_landmark in hand_landmarks:
                mp_drawing.draw_landmarks(frame, hand_landmark, mp_hands.HAND_CONNECTIONS)  # Dibujar las marcas de la mano
        ...
        if capturing:
            save_data(keypoints, label_id, STATIC_MODEL_v3_KEYPOINTS_DIR)  # Guardar los datos capturados
            ...
            count_samples_for_label(STATIC_MODEL_v3_KEYPOINTS_DIR, label_id)  # Muestra el conteo de muestras
